...
layui.define(['layer', 'form'], function (exports) {
    ...
    var setIframe = function (editor, textArea, set) {
                ...
                , toolEvent = {
                                ...
                                layui.use('upload', function (upload) {
                                    ...
                                    upload.render({
                                        ...
                                        , done: function (res, input, upload) {
                                            layer.close(loding);
                                            if (res.code == 0) {
                                                Imgsrc.val(res.filename);
                                                altStr.val(res.filename);
                                            } else {
                                                var curIndex = layer.open({
                                                    ...
                                                    , area: ['390px', '260px']
                                                    , offset: 't'
                                                    , content: res.msg + "<div style='text-align:center;'><img src='" + res.filename + "' style='max-height:80px'/></div><p style='text-align:center'>确定使用该文件吗？</p>"
                                                    , btn: ['确定', '取消']
                                                    , yes: function () {
                                                        Imgsrc.val(res.filename);
                                                        altStr.val(res.filename);
                                        ...
                                        , done: function (res, input, upload) {
                                            ...
                                            if (res.code == 0) {
                                                ...
                                                var curIndex = layer.open({
                                                    ...
                                                    , area: ['390px', '260px']
                                                    , offset: 't'
                                                    , content: res.msg + "<div><img src='" + res.filename + "' style='max-height:100px'/></div><label class='layui-form-label'>确定使用该文件吗？</label>"
                                                    , btn: ['确定', '取消']
                                                    , yes: function () {
                                                        res.data = res.data || {};
                                                        cover.val(res.filename);
                                        ...
                                        , done: function (res, input, upload) {
                                            ...
                                            if (res.code == 0) {
                                                ...
                                                var curIndex = layer.open({
                                                    ...
                                                    , area: ['390px', '260px']
                                                    , offset: 't'
                                                    , content: res.msg + "<div><video src='" + res.filename + "' style='max-height:100px' controls='controls'/></div>确定使用该文件吗？"
                                                    , btn: ['确定', '取消']
                                                    , yes: function () {
                                                        video.val(res.filename);
                                                        layer.close(curIndex);
                    ...
                    audio: function (range) {
                        ...
                        layer.open({
                            ...
                            , offset: '100px'
                            , skin: 'layui-layer-border'
                            , content: ['<ul class="layui-form layui-form-pane" style="margin: 20px;">', '<li class="layui-form-item">', '<button type="button" class="layui-btn" id="LayEdit_InsertAudio"> <i class="layui-icon"></i>上传音频</button>', '<input type="text" name="audio" placeholder="请选择文件" style="width: 79%;position: relative;float: right;" class="layui-input">', "</li>",  '<li class="layui-form-item" style="text-align: center;">', '<button type="button" lay-submit  class="layui-btn layedit-btn-yes"> 确定 </button>', '<button style="margin-left: 20px;" type="button" class="layui-btn layui-btn-primary"> 取消 </button>', "</li>", "</ul>"].join("")
                            , success: function (layero, index) {

                                layui.use('upload', function (upload) {
                                    var loding, audio = layero.find('input[name="audio"]'), cover = layero.find('input[name="cover"]');
                                    var upload = layui.upload;
                                    var uploadfile = set.uploadAudio || {};
                                    //执行实例
                                    upload.render({
                                        ...
                                        , done: function (res, input, upload) {
                                            ...
                                            if (res.code == 0) {
                                                ...
                                                var curIndex = layer.open({
                                                    ...
                                                    , area: ['390px', '260px']
                                                    , offset: 't'
                                                    , content: res.msg + "<div><audio src='" + res.filename + "' controls='controls'/></div>确定使用该文件吗？"
                                                    , btn: ['确定', '取消']
                                                    , yes: function () {
                                                        audio.val(res.filename);
                                                        layer.close(curIndex);
                                    ...
                                    layero.find('.layedit-btn-yes').on('click', function () {
                                        var container = getContainer(range)
                                            , parentNode = $(container).parent();
                                        insertInline.call(iframeWin, 'p', {
                                            text: '&nbsp;<audio src="' + audio.val() + '" poster="' + cover.val() + '" controls="controls" >您的浏览器不支持audio播放</audio>&nbsp;'
                                        }, range);
                                        layer.close(index);
                                    });
                            })
                                ...
                                success: function (layero, index) {
                                    ...
                                    layero.find('.layui-btn-danger').on('click', function () {
                                        if (currenNode.tagName == "BODY") {
                                            ...
                                            if (callDel.url != "") {
                                                $.post(callDel.url, { "videopath": event.target.src, "imgpath": event.target.poster }, function (r) {
                                                    parentNode.remove();
                                                })
                                            } else {
                                                parentNode.remove();
                                        ...
                                        } else if (currenNode.tagName == "AUDIO"){
                                            if (callDel.url != "") {
                                                $.post(callDel.url, { "audiopath": event.target.src }, function (r) {
                                                    currenNode.remove();
                                                })
                                            } else {
                                                currenNode.remove();
        ...
        , tools = {
            ...
            , audio: '<i class="layui-icon layui-icon-headset" title="插入音频" layedit-event="audio" style="font-size:18px"></i>'
